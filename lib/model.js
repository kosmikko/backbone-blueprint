...
var Model = Backbone.Model.extend({
  ...
  toJSON: function(options) {
    ...
    function convertProperty(property, name) {
      ...
      if(this.relationDefinitions[name]) {
        ...
        if(options.recursive && typeof attribute.toJSON === 'function') {
          ...
          if(options.projection && options.projection[name]) {
            value = _.pick.apply(null, [value, options.projection[name]]);
          }
        }
      } else {
        value = attribute;
    ...
    if(options.projection && options.projection.removeFields) {
      _.each(options.projection.removeFields, function(fieldToRemove) {
